...
/*  Copyright 2015 Imaginary Media (email : support@jmr.codes)
...
function imeg_embed_gist($atts, $content = null) {
  ...
  $val = shortcode_atts(array( 'src' => '', 'height' => '' ), $atts);
  $html = '';

  if ( ( ! $val[src] ) || ( $val['src'] == '' ) ) {
    // If no script src present, exit
    return;
  } else {
    $html .= '<script src="' . $val['src'] . '"></script>';
    if ( $val['height'] !== '' ) {
      $html .= '<style>.gist .gist-data{ max-height:' . $val['height'] . ' !important;}</style>';
    }
    return $html;
  }
}
